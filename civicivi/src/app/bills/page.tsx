...
import { Search, SlidersHorizontal, TrendingUp, X, ArrowRight, Building2, Users } from "lucide-react";
import type { Bill } from "@/lib/types";

const ALL_TOPICS = Array.from(
  new Set(MOCK_BILLS.flatMap((b) => b.topics))
).sort();
...
export default function BillsPage() {
  ...
  const filteredBills = useMemo(() => {
    ...
    if (selectedTopics.length > 0) {
      bills = bills.filter((b) => selectedTopics.some((t) => b.topics.includes(t)));
    }

    if (selectedStatuses.length > 0) {
      bills = bills.filter((b) => selectedStatuses.includes(b.status));
    }
  ...
  }, [search, selectedTopics, selectedStatuses, sortBy]);
      ...
      <main className="flex-1 pt-24 pb-16 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search bills by title, topic, or keyword..."
          ...
          <p className="text-slate-500 text-sm mb-4">
            Showing <span className="text-slate-300 font-semibold">{filteredBills.length}</span> bill
          ...
          {filteredBills.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 text-sm">Try different search terms or clear your filters.</p>
            </div>
          ) : (
